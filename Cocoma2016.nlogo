__includes ["communication.nls" "fonctionsUtiles.nls" "ennemis.nls" "convoi.nls" "drones.nls"]
...
globals [mapAlt solAlt basseAlt hauteAlt ; variables topologiques Z discretise: definit le niveau ou se trouvent toutes les informations de la carte (obstacles base etc.) car en 2D, niveau au sol ou se trouvent les agents, niveau basse altitude et niveau haute altitude
  ...
  fuel-dec            ; La quantité d'essence consommée à chaque tick par les drones
  mission-completed?  ; indique si la mission a été accomplie avec succès
  mission-failed?     ; indique si la mission a échoué
  landing-patch       ; premier patch de la piste d'aterissage
  refill-patch        ; Patch pour faire le plein de carburant
  full-convoi?        ; Indique si le convoi ne s'est pas encore séparé pour diversion
  drone-speed         ; Vitesse d'avancement maximale d'un drone
  convoi-speed        ; Vitesse d'avancement maximale d'un véhicule du convoi
  maxdir-convoi       ; maximal max angle
  time-between-shots  ; Nombre de ticks entre deux tirs consécutifs
  ]

patches-own [
  obstacle? base? hangar? objectif? bridge? mountain?; variables topologiques au niveau mapAlt, permet de definir les patchs praticables et ceux qui sont des obstacles
  landing? take-off? ; variables topologiques indiquant si c'est la piste d'aterissage ou de decolage
...
to setup-env
  ask patches
  [
    set obstacle? false set base? false set hangar? false set objectif? false
    set bridge? false set take-off? false set landing? false
  ]
  ...
  ; Montagnes
  if nb-mountains > 0 [ ask n-of nb-mountains patches with [pzcor = mapAlt and pxcor > 9 and pycor > 9] [ask patches with [distance-nowrap myself < 4] [set pcolor brown set obstacle? true]] ]
  ...
  ask patches with[pzcor = solAlt and pxcor >= 3 and pxcor < 5 and pycor >= 0 and pycor < 12][set take-off? true]
  ...
  ask patches with[pzcor = solAlt and pycor = 0 and pxcor >= 0 and pxcor < 18][ set landing? true]
  set landing-patch patch 17 0 (solAlt + 1)
  set refill-patch patch 5 0 solAlt
...
to go
  ...
  if check-win = 0 [ user-message "Les ennemis ont gagné" stop ]
  if check-win = 1 [ user-message "Bravo la voiture à protéger a atteint l'objectif !" stop ]
...
Ceci est un modèle de simulation de convoi, escorté par des drones, dont une voiture doit arriver à un objectif sur la carte sans se faire tuer par des véhicules ennemis disséminés sur la carte

## HOW IT WORKS

Le convoi détermine et suit un chemin jusqu'à l'objectif. Certain des drones, les drones protecteurs,  protègent le convoi. D'autres, les drones chasseurs, "déblaient" le chemin jusqu'à l'objectif.
Les drones doivent en plus régulièrement rentrer a la base pour se ravitailler en munition et essence

## HOW TO USE IT

Pour intialiser la carte ainsi que les positions des agents, cliquer sur setup
Pour lancer la simulation, cliquer sur go

Les boites d'input en haut permet de régler le nombre d'agents (voiture du convoi, drones, ennemis) ou d'élements de l'environnement (rivières, montagnes, lacs)

Les sliders à droite permettent de régler divers paramètres pour chaque type d'agent

Les paramètres en bas à gauche concernent l'algorithme A*, qui est utilisé pour le calcul de chemin dans cette simulation

## THINGS TO NOTICE

Le convoi se sépare en deux : une moitié qui rentre à la base, l'autre qui va vers l'objectif.
Les drones vont plus souvent se ravitailler lorsque leurs reserves (munitions, fuel) sont basses


## THINGS TO TRY

Faire varier la portée de tir des ennemis et des drones, observer leur influence sur le succès de la mission

## EXTENDING THE MODEL

Gérer les cas où les communications sont limitées dans l'espace.
Utiliser les observations des drones pour prévoir un chemin moins dangereux pour le convoi
@#$#@#$#@
default
true
0
